...
const tabs = [
  ...
  { label: "SOPs", slug: "/sop", icon: FileText },
  { label: "Recommendations", slug: "/schedule", icon: Lightbulb },
  { label: "Optimization", slug: "/opt", icon: BarChart3 },
  { label: "Agents", slug: "/agent", icon: Bot },
  { label: "Audit", slug: "/audit", icon: ClipboardList },
];

export default function DCDetailLayoutClient({
  children,
}: {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#001C77] text-white rounded-lg hover:bg-[#002299] transition-colors text-sm font-medium"
          >
            Back to Sites
          </Link>
        </div>
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-[1440px] mx-auto px-6">
          <div className="flex items-center gap-2 pt-4 pb-2 text-sm">
            <Link href="/dc" className="text-slate-400 hover:text-[#001C77] transition-colors">
              Sites
            </Link>
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-4 h-4 ${
                      active ? "text-[#001C77]" : "text-slate-400 group-hover:text-slate-500"
                    }`}
                  />
                  {tab.label}
                </Link>
